...
tmp<-stack("C:\\Workspace\\RMardown_NAPS\\Open-NAPs-Data\\opennaps\\cru_ts4.05.1901.2020.tmp.dat.nc")
tcrop<-raster::crop(tmp, sleone0)
...
plotly::plot_ly(type= 'bar', data= pr_tmp, x= ~Month, y= ~pr, name = 'Precipitation')%>%
  plotly::add_lines(x= ~Month, y= ~tmean, mode = 'lines+markers',col='#D21919', name= 'Temperature', yaxis='y2')%>%
  #plotly::add_markers(x= ~Month, y= ~tmean, pch=1,color='#D21919', yaxis='y2', name='')%>%
    plotly::layout(legend=list(orientation='h', y=-0.18,x=0.3), yaxis=list(title='Precipitation (mm)',showticklables=F, range=c(0,600)),width=600, height=500, title='Sierra Leone \n (Mean 1981-2020)', yaxis2=ty)
...
```{r ggplot-monthly-ts, echo=FALSE}

#pr_monthly<-prdf%>% group_by(Month)%>%
 # summarise(across(contains("pr"), ~mean(pr))) 
#pr_monthly$Month<-factor(pr_monthly$Month,levels = c('06','07','08','09','10','11','12','01','02','03','04','05')) # reorder the months to start plotting from June to January
...
#tmp_monthly<-tmpdf%>%group_by(Month)%>%summarise('tmean'=mean(tmean))
...
#coeff<-0.1
#pr_tmp%>% ggplot(aes(x=Month))+
#geom_col(aes(y=pr))+
 #geom_line(aes(y=tmean/coeff), size=9, color='red')+
  #scale_y_continuous(name='Precipitation (mm)', sec.axis = sec_axis(trans=~.*coeff, name='Temperature'))
...
```{r ssp245-national}
tn245.40<-raster("wc2.1_2.5m_tmin_BCC-CSM2-MR_ssp245_2021-2040.tif")%>%crop(sleone0)%>%mask(sleone0)
tx245.40<-raster("wc2.1_2.5m_tmax_BCC-CSM2-MR_ssp245_2021-2040.tif")%>%crop(sleone0)%>%mask(sleone0)
...
```{r ssp245-coast}
tn245.40_coast<-raster("wc2.1_2.5m_tmin_BCC-CSM2-MR_ssp245_2021-2040.tif")%>%crop(slcoast)%>%mask(slcoast)
tx245.40_coast<-raster("wc2.1_2.5m_tmax_BCC-CSM2-MR_ssp245_2021-2040.tif")%>%crop(slcoast)%>%mask(slcoast)
tmn245.40_coast<-(tn245.40_coast+tx245.40_coast)/2
#plot(tmn245.40_coast)
tvar245_coast<-basemn-tmn245.40_coast


#sf_coast<-st_as_sf(slcoast)
plot(tvar245_coast, xlab='Longitude', ylab='Latitude')
...
```{r ssp245-inland}
tn245.40_inland<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/tmin245/wc2.1_2.5m_tmin_BCC-CSM2-MR_ssp245_2021-2040.tif")%>%crop(slinland)%>%mask(slinland)
tx245.40_inland<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/tmax245/wc2.1_2.5m_tmax_BCC-CSM2-MR_ssp245_2021-2040.tif")%>%crop(slinland)%>%mask(slinland)
tmn245.40_inland<-(tn245.40_inland+tx245.40_inland)/2
#plot(tmn245.40_inland)
tvar245_inland<-basemn-tmn245.40_inland

sf_inland<-st_as_sf(slinland)
plot(tvar245_inland, xlab='Longitude', ylab='Latitude')
...
```{r ssp585}
tn585.40<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/tmin585/wc2.1_2.5m_tmin_BCC-CSM2-MR_ssp585_2021-2040.tif")%>%crop(sleone0)%>%mask(sleone0)
tx585.40<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/tmax585/wc2.1_2.5m_tmax_BCC-CSM2-MR_ssp585_2021-2040.tif")%>%crop(sleone0)%>%mask(sleone0)
...
```{r ssp585-coast}
tn585.40_coast<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/tmin585/wc2.1_2.5m_tmin_BCC-CSM2-MR_ssp585_2021-2040.tif")%>%crop(slcoast)%>%mask(slcoast)
tx585.40_coast<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/tmax585/wc2.1_2.5m_tmax_BCC-CSM2-MR_ssp585_2021-2040.tif")%>%crop(slcoast)%>%mask(slcoast)
tmn585.40_coast<-(tn585.40_coast+tx585.40_coast)/2
tvar585_coast<-basemn-tmn585.40_coast
plot(tvar585_coast, xlab='Longitude', ylab='Latitude')
plot(admn2.coast, add=TRUE)
```
...
```{r ssp585-inland}
tn585.40_inland<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/tmin585/wc2.1_2.5m_tmin_BCC-CSM2-MR_ssp585_2021-2040.tif")%>%crop(slinland)%>%mask(slinland)
tx585.40_inland<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/tmax585/wc2.1_2.5m_tmax_BCC-CSM2-MR_ssp585_2021-2040.tif")%>%crop(slinland)%>%mask(slinland)
tmn585.40_inland<-(tn585.40_inland+tx585.40_inland)/2
tvar585_inland<-basemn-tmn585.40_inland
plot(tvar585_inland, xlab='Longitude', ylab='Latitude')
plot(admn2.inland, add=TRUE)
```
...
```{r pr126-national}
pr126.40<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/pr126/wc2.1_2.5m_prec_BCC-CSM2-MR_ssp126_2021-2040.tif")%>%crop(sleone0)%>%mask(sleone0)
#plot(pr126.40)
...
```{r pr126-coast}
pr126.40_coast<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/pr126/wc2.1_2.5m_prec_BCC-CSM2-MR_ssp126_2021-2040.tif")%>%crop(slcoast)%>%mask(slcoast)
#plot(pr126.40)
prvar126_coast<-basepr-pr126.40_coast
...
```{r pr126-inland}
pr126.40_inland<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/pr126/wc2.1_2.5m_prec_BCC-CSM2-MR_ssp126_2021-2040.tif")%>%crop(slinland)%>%mask(slinland)
#plot(pr126.40)
prvar126_inland<-basepr-pr126.40_inland
...
```{r pr245-national}
pr245.40<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/pr245/wc2.1_2.5m_prec_BCC-CSM2-MR_ssp245_2021-2040.tif")%>%crop(sleone0)%>%mask(sleone0)
#plot(pr245.40)
...
```{r pr245-coast}
pr245.40_coast<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/pr245/wc2.1_2.5m_prec_BCC-CSM2-MR_ssp245_2021-2040.tif")%>%crop(slcoast)%>%mask(slcoast)
#plot(pr245.40)
prvar245_coast<-basepr-pr245.40_coast
...
```{r pr245-inland}
pr245.40_inland<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/pr245/wc2.1_2.5m_prec_BCC-CSM2-MR_ssp245_2021-2040.tif")%>%crop(slinland)%>%mask(slinland)
#plot(pr245.40)
prvar245_inland<-basepr-pr245.40_inland
...
```{r pr-585-national}
pr585.40<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/pr585/wc2.1_2.5m_prec_BCC-CSM2-MR_ssp585_2021-2040.tif")%>%crop(sleone0)%>%mask(sleone0)
#plot(pr585.40)
...
```{r pr-585-coast}
pr585.40_coast<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/pr585/wc2.1_2.5m_prec_BCC-CSM2-MR_ssp585_2021-2040.tif")%>%crop(slcoast)%>%mask(slcoast)
#plot(pr585.40)
prvar585_coast<-basepr-pr585.40_coast
...
```{r pr-585-inland}
pr585.40_inland<-raster("C:/Workspace/RMardown_NAPS/Open-NAPs-Data/opennaps/WC_Fut_data/pr585/wc2.1_2.5m_prec_BCC-CSM2-MR_ssp585_2021-2040.tif")%>%crop(slinland)%>%mask(slinland)
#plot(pr585.40)
prvar585_inland<-basepr-pr585.40_inland
